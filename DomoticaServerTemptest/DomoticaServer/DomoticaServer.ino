...
#define unitCodeApa3      22609154  // replace with your own code
...
byte mac[] = { 0x78, 0x6c, 0x8f, 0x36, 0x84, 0x8a }; // Ethernet adapter shield S. Oosterhaven
...
int TempValue = 0;                    // Variable voor temperatuur
int HumiValue = 0;                    // Variable voor vochtigheid
int apaState0 = 0;
int apaState1 = 0;
int apaState2 = 0;
...
void loop()
{
   ...
   while (ethernetClient.connected()) 
   {
      ...
      TempValue = th.t*0.95;         // update temp
      HumiValue = th.h*0.95;         //update vocht
...
{   
   delay(100);
   apa3Transmitter.sendUnit(0, apaState0);          // APA3 Kaku (Gamma)
   apa3Transmitter.sendUnit(1, apaState1);          // APA3 Kaku (Gamma)   
   apa3Transmitter.sendUnit(2, apaState2);          // APA3 Kaku (Gamma)                   
   delay(100);
...
{     
         ...
         case 'a': // Temperatuur naar app 
            ToCharBuf2(TempValue, buf, 4);                // convert to charbuffer
            server.write(buf, 4);                             // response is always 4 chars (\n included)
            Serial.print("Temp: "); Serial.println(buf);
            break;
          case 'b': // Vochtigheid naar app  
            ToCharBuf3(HumiValue, buf, 4);                // convert to charbuffer
            server.write(buf, 4);                             // response is always 4 chars (\n included)
...
void ToCharBuf2(int val, char buf[], int len)
{
   String s;
   s = String(val);                        // convert tot string
   if (s.length() == 1) s = " " + s;       // prefix redundant "0" 
...
void ToCharBuf3(int val, char buf[], int len)
{
   String s;
   s = String(val);                        // convert tot string
   if (s.length() == 1) s = " " + s;       // prefix redundant "0" 
