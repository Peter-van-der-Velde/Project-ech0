...
bool up = false;
bool down = false;
bool start = false; // true if it starts the circuit
...
void slagboom(){
  ...
  if(distance<detectie_distance && !start){       // barrier down in case the barrier doesn't start the circuit
  ...
  else if(distance>detectie_distance && start){       // barrier down in case the barrier does start the circuit
    for (pos = pos; pos > 85; pos -= 1) { // goes from pos degrees to 85 degrees
    myservo.write(pos);              // tell servo to go to position in variable 'pos'
    delay(15);                       // waits 15ms for the servo to reach the position
    down = true;
